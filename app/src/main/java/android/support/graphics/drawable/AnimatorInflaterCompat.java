    ...
    private static void dumpKeyframes(Object[] keyframes, String header) {
        if (keyframes != null && keyframes.length != 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                Log.d(TAG, "Keyframe " + i + ": fraction " + (keyframe.getFraction() < 0.0f ? Constants.NULL_VERSION_ID : Float.valueOf(keyframe.getFraction())) + ", " + ", value : " + (keyframe.hasValue() ? keyframe.getValue() : Constants.NULL_VERSION_ID));
            }
        }
    }
