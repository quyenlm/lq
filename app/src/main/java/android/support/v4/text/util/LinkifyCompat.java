...
public final class LinkifyCompat {
    ...
    public static final boolean addLinks(@NonNull Spannable text, int mask) {
        ...
        if ((mask & 2) != 0) {
            gatherLinks(links, text, PatternsCompat.AUTOLINK_EMAIL_ADDRESS, new String[]{WebView.SCHEME_MAILTO}, (Linkify.MatchFilter) null, (Linkify.TransformFilter) null);
        }
        if ((mask & 8) != 0) {
    ...
    private static final void gatherMapLinks(ArrayList<LinkSpec> links, Spannable s) {
        ...
        while (true) {
            try {
                String address = android.webkit.WebView.findAddress(string);
