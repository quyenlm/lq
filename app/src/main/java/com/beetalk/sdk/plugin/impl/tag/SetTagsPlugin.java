    ...
    public void executeAction(final Activity activity, final TagsData data) {
        Task.callInBackground(new Callable<PluginResult>() {
            public PluginResult call() throws Exception {
                ...
                try {
                    pluginResult.flag = BeePostAPI.setTags(activity.getApplicationContext(), data.mAppId, data.mAppKey, String.valueOf(DeviceUtil.generateDeviceId(activity)), data.mTags) ? 0 : GGErrorCode.NETWORK_EXCEPTION.getCode().intValue();
                } catch (IOException e) {
