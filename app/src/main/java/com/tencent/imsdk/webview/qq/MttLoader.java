...
public class MttLoader {
    ...
    public static String getValidQBUrl(Context context, String url) {
        if (!url.toLowerCase(Locale.getDefault()).startsWith("qb://")) {
            return url;
        }
    ...
    public static int loadUrl(Activity context, String url, HashMap<String, String> args) {
        ...
        try {
            ...
            if (uri.getScheme().toLowerCase(Locale.getDefault()).equals("qb") && !isSupportQBScheme(context)) {
    ...
    private static BrowserPackageInfo chooseClassName(Context context, Uri uri) {
        ...
        BrowserPackageInfo info = new BrowserPackageInfo();
        ...
        try {
            ...
            try {
                ...
                if (pi != null && pi.versionCode > VERSION_420) {
                    ...
                    result.quahead += pi.versionName.replaceAll("\\.", "");
                    return result;
                }
    ...
    private static boolean hasValidProtocal(String aUrl) {
        ...
        int pos1 = url.toLowerCase(Locale.getDefault()).indexOf("://");
        int pos2 = url.toLowerCase(Locale.getDefault()).indexOf(46);
        if (pos1 <= 0 || pos2 <= 0 || pos1 <= pos2) {
            return url.toLowerCase(Locale.getDefault()).contains("://");
        }
        return false;
    }
