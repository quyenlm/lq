...
public class CircleImageView extends ImageView {
    ...
    public CircleImageView(Context context) {
        ...
        this.mEventListener = new IEventListener() {
            public void onEvent(int eventId, Event event) {
                Bitmap bitmap;
                EventGetCDNFileFinish evt = (EventGetCDNFileFinish) event;
                if (CircleImageView.this.mUrl.equals(evt.url)) {
                    if (!(evt.data == null || (bitmap = BitmapFactory.decodeByteArray(evt.data, 0, evt.data.length)) == null)) {
    ...
    public void setImageUrl(String url) {
        ...
        EventManager.getInstance().post(3001, new EventGetCDNFile(this.mUrl));
    }
