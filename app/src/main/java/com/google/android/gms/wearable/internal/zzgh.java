...
public class zzgh {
    ...
    public static zzgh zzbz(Context context) {
        zzbo.zzu(context);
        synchronized (zzgh.class) {
            if (zzbTC == null) {
                zzbTC = new zzgh(context);
            }
        }
        return zzbTC;
    }

    private static boolean zzc(PackageInfo packageInfo, boolean z) {
        if (packageInfo.signatures.length != 1) {
            Log.w("WearSignatureVerifier", "Package has more than one signature.");
            return false;
        }
        ...
        if (z) {
            return zza(packageInfo, zzbTD, zzbTE);
        }
        return zza(packageInfo, zzbTD);
    }

    private static byte[] zzcs(String str) {
        try {
    ...
    private final PackageInfo zzgn(String str) {
        try {
            return zzbha.zzaP(this.mContext).getPackageInfo(str, 64);
    ...
    public final boolean zzgm(String str) {
        PackageInfo zzgn = zzgn(str);
        if (zzgn == null) {
            return false;
        }
        if (zzo.zzaw(this.mContext)) {
            return zzc(zzgn, true);
        }
        boolean zzc = zzc(zzgn, false);
        if (zzc || !zzc(zzgn, true)) {
            return zzc;
        }
