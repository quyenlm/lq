...
public interface HistoryApi {
    ...
    public static class ViewIntentBuilder {
        ...
        public Intent build() {
            ...
            intent2.setType(DataType.getMimeType(this.zzaTa.getDataType()));
            intent2.putExtra(Fitness.EXTRA_START_TIME, this.zzaTb);
            intent2.putExtra(Fitness.EXTRA_END_TIME, this.zzaTc);
            zze.zza(this.zzaTa, intent2, DataSource.EXTRA_DATA_SOURCE);
            if (this.zzaTd == null || (resolveActivity = this.zzqD.getPackageManager().resolveActivity(intent, 0)) == null) {
                return intent2;
            }
            (intent = new Intent(intent2).setPackage(this.zzaTd)).setComponent(new ComponentName(this.zzaTd, resolveActivity.activityInfo.name));
            return intent;
        }
