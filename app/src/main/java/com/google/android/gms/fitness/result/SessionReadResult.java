...
public class SessionReadResult extends zza implements Result {
    ...
    public boolean equals(Object obj) {
        if (this != obj) {
            ...
            if (!(this.mStatus.equals(sessionReadResult.mStatus) && zzbe.equal(this.zzaWs, sessionReadResult.zzaWs) && zzbe.equal(this.zzaXB, sessionReadResult.zzaXB))) {
                return false;
            }
        }
        return true;
    ...
    public List<DataSet> getDataSet(Session session) {
        ...
        for (zzae next : this.zzaXB) {
            if (zzbe.equal(session, next.getSession())) {
    ...
    public List<DataSet> getDataSet(Session session, DataType dataType) {
        ...
        ArrayList arrayList = new ArrayList();
        for (zzae next : this.zzaXB) {
            if (zzbe.equal(session, next.getSession()) && dataType.equals(next.getDataSet().getDataType())) {
                arrayList.add(next.getDataSet());
            }
        }
        return arrayList;
    ...
    public void writeToParcel(Parcel parcel, int i) {
        ...
        zzd.zza(parcel, 3, (Parcelable) getStatus(), i, false);
