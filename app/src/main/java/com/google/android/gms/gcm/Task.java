...
public class Task implements ReflectedParcelable {
        ...
        public void checkConditions() {
            ...
            if (zzi != null) {
                ...
                if (zzvE == 1 || zzvE == 0) {
                    ...
                    int zzvG = zzi.zzvG();
                    if (zzvE == 0 && zzvF < 0) {
                        throw new IllegalArgumentException(new StringBuilder(52).append("InitialBackoffSeconds can't be negative: ").append(zzvF).toString());
                    } else if (zzvE == 1 && zzvF < 10) {
                        throw new IllegalArgumentException("RETRY_POLICY_LINEAR must have an initial backoff at least 10 seconds.");
                    } else if (zzvG < zzvF) {
                        throw new IllegalArgumentException(new StringBuilder(77).append("MaximumBackoffSeconds must be greater than InitialBackoffSeconds: ").append(zzi.zzvG()).toString());
                    }
                } else {
                    throw new IllegalArgumentException(new StringBuilder(45).append("Must provide a valid RetryPolicy: ").append(zzvE).toString());
                }
            }
            if (this.isPersisted) {
    ...
    Task(Parcel parcel) {
        ...
        this.zzbgi = parcel.readInt() != 1 ? false : z;
        this.zzbgj = 2;
        this.zzbgk = false;
        this.zzbgl = false;
