...
public class AWS4Signer extends AbstractAWSSigner implements Presigner, RegionAwareSigner, ServiceAwareSigner {
    ...
    public void sign(Request<?> request, AWSCredentials credentials) {
        if (!(credentials instanceof AnonymousAWSCredentials)) {
            ...
            request2.addHeader("Authorization", "AWS4-HMAC-SHA256 " + ("Credential=" + (sanitizedCredentials.getAWSAccessKeyId() + Constants.URL_PATH_DELIMITER + scope)) + ", " + ("SignedHeaders=" + getSignedHeadersString(request)) + ", " + ("Signature=" + BinaryUtils.toHex(headerSigningResult.getSignature())));
    ...
    public final HeaderSigningResult computeSignature(Request<?> request, String dateStamp, String timeStamp, String algorithm, String contentSha256, AWSCredentials sanitizedCredentials) {
        ...
        String scope = dateStamp + Constants.URL_PATH_DELIMITER + regionName2 + Constants.URL_PATH_DELIMITER + serviceName2 + Constants.URL_PATH_DELIMITER + TERMINATOR;
    ...
    public String getScope(Request<?> request, String dateStamp) {
        String regionName2 = extractRegionName(request.getEndpoint());
        return dateStamp + Constants.URL_PATH_DELIMITER + regionName2 + Constants.URL_PATH_DELIMITER + extractServiceName(request.getEndpoint()) + Constants.URL_PATH_DELIMITER + TERMINATOR;
    }
    ...
    public void presignRequest(Request<?> request, AWSCredentials credentials, Date expiration) {
        if (!(credentials instanceof AnonymousAWSCredentials)) {
            ...
            request.addParameter("X-Amz-Credential", sanitizedCredentials.getAWSAccessKeyId() + Constants.URL_PATH_DELIMITER + getScope(request, dateStamp));
            request.addParameter("X-Amz-Signature", BinaryUtils.toHex(computeSignature(request, dateStamp, timeStamp, ALGORITHM, calculateContentHashPresign(request), sanitizedCredentials).getSignature()));
        }
    }
    ...
    public boolean needsSign(String header) {
        return header.equalsIgnoreCase("date") || header.equalsIgnoreCase(Headers.CONTENT_MD5) || header.equalsIgnoreCase(StatHelper.StatParams.HOST) || header.startsWith("x-amz") || header.startsWith("X-Amz");
    }
}
