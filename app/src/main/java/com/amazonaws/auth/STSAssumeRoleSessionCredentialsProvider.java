...
public class STSAssumeRoleSessionCredentialsProvider implements AWSCredentialsProvider {
    ...
    private void startSession() {
        Credentials stsCredentials = this.securityTokenService.assumeRole(new AssumeRoleRequest().withRoleArn(this.roleArn).withDurationSeconds(900).withRoleSessionName(this.roleSessionName)).getCredentials();
    ...
    private boolean needsNewSession() {
        if (this.sessionCredentials != null && this.sessionCredentialsExpiration.getTime() - System.currentTimeMillis() >= Constants.WATCHDOG_WAKE_TIMER) {
            return false;
        }
        return true;
    }
