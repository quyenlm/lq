...
public class DefaultRequest<T> implements Request<T> {
    ...
    public DefaultRequest(String serviceName2) {
        this((AmazonWebServiceRequest) null, serviceName2);
    }
    ...
    public String toString() {
        ...
        if (resourcePath2 == null) {
            builder.append(Constants.URL_PATH_DELIMITER);
        } else {
            if (!resourcePath2.startsWith(Constants.URL_PATH_DELIMITER)) {
                builder.append(Constants.URL_PATH_DELIMITER);
            }
        ...
        if (!getParameters().isEmpty()) {
            ...
            for (String key : getParameters().keySet()) {
                builder.append(key).append(": ").append(getParameters().get(key)).append(", ");
            }
            builder.append(") ");
        }
        if (!getHeaders().isEmpty()) {
            ...
            for (String key2 : getHeaders().keySet()) {
                builder.append(key2).append(": ").append(getHeaders().get(key2)).append(", ");
            }
            builder.append(") ");
        }
