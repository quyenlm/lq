...
public class XMLWriter {
    ...
    private String escapeXMLEntities(String s) {
        if (s.contains(HttpRequest.HTTP_REQ_ENTITY_JOIN)) {
            s = s.replace("&quot;", "\"").replace("&apos;", "'").replace("&lt;", "<").replace("&gt;", ">").replace("&amp;", HttpRequest.HTTP_REQ_ENTITY_JOIN);
        }
        return s.replace(HttpRequest.HTTP_REQ_ENTITY_JOIN, "&amp;").replace("\"", "&quot;").replace("'", "&apos;").replace("<", "&lt;").replace(">", "&gt;");
    }
}
