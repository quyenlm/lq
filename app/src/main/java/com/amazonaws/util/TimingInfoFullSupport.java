...
class TimingInfoFullSupport extends TimingInfo {
    ...
    public void addSubMeasurement(String subMeasurementName, TimingInfo ti) {
        ...
        if (ti.isEndTimeKnown()) {
            ...
            LogFactory.getLog((Class) getClass()).debug("Skip submeasurement timing info with no end time for " + subMeasurementName);
        }
    }
    ...
    public TimingInfo getLastSubMeasurement(String subMeasurementName) {
        ...
        if (this.subMeasurementsByName == null || this.subMeasurementsByName.size() == 0 || (timings = this.subMeasurementsByName.get(subMeasurementName)) == null || timings.size() == 0) {
            return null;
        }
