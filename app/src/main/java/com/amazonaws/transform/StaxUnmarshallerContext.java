...
public class StaxUnmarshallerContext {
    ...
    public boolean testExpression(String expression, int startingStackDepth) {
        ...
        int index = -1;
        while (true) {
            index = expression.indexOf(Constants.URL_PATH_DELIMITER, index + 1);
            if (index <= -1) {
                break;
        ...
        if (getCurrentDepth() != startingStackDepth || !this.stackString.endsWith(Constants.URL_PATH_DELIMITER + expression)) {
            return false;
        }
        return true;
    }
    ...
    private void updateContext() {
        if (this.currentEventType == 2) {
            this.stackString += Constants.URL_PATH_DELIMITER + this.xpp.getName();
