...
class QueryParser {
    private static final String[] AttributeEvals = {HttpRequest.HTTP_REQ_ENTITY_MERGE, "!=", "^=", "$=", "*=", "~="};
    ...
    private String consumeSubQuery() {
        ...
        while (!this.tq.isEmpty()) {
            if (this.tq.matches(h.a)) {
                sq.append(h.a).append(this.tq.chompBalanced('(', ')')).append(h.b);
    ...
    private void byAttribute() {
        ...
        } else if (cq.matchChomp(HttpRequest.HTTP_REQ_ENTITY_MERGE)) {
    ...
    private void cssNthChild(boolean backwards, boolean ofType) {
        int b = 0;
        int a = 1;
        String argS = this.tq.chompTo(h.b).trim().toLowerCase();
    ...
    private int consumeIndex() {
        String indexS = this.tq.chompTo(h.b).trim();
